...
describe('uploader', () => {
  ...
  describe('must return the right drag-and-drop label', () => {
    ...
    it('should trigger events', async () => {
      const spy = jest.spyOn(wrapper.vm.$refs.uploader, 'dispatchEvent');
      await wrapper.setData({
        tokens: [],
      });
      expect(spy).toHaveBeenCalled();
      expect(spy.mock.calls[0][0].type).toEqual('osisdocument:delete');
      ...
      expect(spy).toHaveBeenCalled();
      expect(spy.mock.calls[0][0].type).toEqual('osisdocument:add');
      expect(spy.mock.calls[0][0].detail).toEqual({
        newTokens: {
          '0': 'foo',
        },
  ...
  it('should clear all tokens when an input is emptied', async () => {
    ...
    expect(Object.values(wrapper.vm.tokens)).toHaveLength(0);
  });
});
